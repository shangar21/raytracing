...
class Camera {
	public:
		...
		void render_ppm(const Hittable& world){
			...
			for(int j = 0; j < image_height; j++){
				for(int i = 0; i < image_width; i++){
					point pixel_center = pixel00_loc + ((double)i * pixel_delta_u) + ((double)j * pixel_delta_v);
		...
		void render_png(const Hittable& world){
			initialize();
			Eigen::MatrixXd R (image_height, image_width);
			Eigen::MatrixXd G (image_height, image_width);
			Eigen::MatrixXd B (image_height, image_width);
			...
			for (int j = 0; j < image_height; j++){
				for (int i = 0; i < image_width; i++){
					...
					R(j, i) = std::clamp(pixel.x(), 0.0, 1.0);
					G(j, i) = std::clamp(pixel.y(), 0.0, 1.0);
					B(j, i) = std::clamp(pixel.z(), 0.0, 1.0);
				}
			}

			writeColorOpenCV(R, G, B, image_height, image_width);
	...
	private:
		...
		void initialize(){
			...
			point viewport_ul = camera_center - point(0.0, 0.0, fl) - viewport_u / 2.0 - viewport_v / 2.0;
