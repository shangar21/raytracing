...
class Hittable {
	public:
		virtual void hit(const Ray<double>& r, double ray_tmin, double ray_tmax, HitRecord& rec) = 0;
};
...
class HittableList: public Hittable {
	public:
		...
		bool hit(const Ray<double>& r, double ray_tmin, double ray_tmax, HitRecord& rec) const override {
			HitRecord temp_rec;
			bool hit_anything = false;
			double closest_so_far = ray_tmax;

			for (hittable_ptr object: objects){
				if (object->hit(r, ray_tmin, ray_tmax, closest_so_far, temp_rec)){
					hit_anything = true;
